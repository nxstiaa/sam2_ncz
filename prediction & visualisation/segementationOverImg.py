...
segmented_mask_dir = "/vol/bitbucket/nc624/sam2/outputs/segmentedMasksFrames_OriginalFrame"
...
for idx, row in tqdm(grouped_first.iterrows(), total=len(grouped_first), desc="Processing videos"): #loop through each row 
    ...
    for frame in framesToSave:
        if frame in video_segments and ann_obj_id in video_segments[frame]:
                ...
                colored_mask[:, :, 3] = mask_resized * 102  # 40% opacity

                # Convert original frame and mask to PIL 
                original_frame_pil = Image.fromarray(original_frame_resized).convert("RGBA")
                color_mask = Image.fromarray(colored_mask)

                #Overlay red mask on original frame
                blended = Image.alpha_composite(original_frame_pil, color_mask)
...
print(f"Exceeded 5 videos processed")
